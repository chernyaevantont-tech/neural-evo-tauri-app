...
### 5. BatchNorm Node (Слой пакетной нормализации)

Нормализует активации, ускоряя обучение и повышая стабильность. Поддерживает как Dense (2D), так и Conv2D (4D) пути.

**Параметры:**
- **Momentum**: Коэффициент для вычисления скользящего среднего (обычно 0.1)
- **Epsilon**: Малое число для числовой стабильности (обычно 1e-5)

### 6. LayerNorm Node (Слой нормализации слоя)

Нормализует активации поперек признаков. Часто используется в рекуррентных сетях или трансформерах.

**Параметры:**
- **Epsilon**: Малое число для числовой стабильности (обычно 1e-5)

### 7. Dropout Node (Слой Dropout)

Случайным образом "выключает" нейроны во время обучения для предотвращения переобучения.

**Параметры:**
- **Prob**: Вероятность исключения нейрона (от 0.0 до 1.0, обычно 0.2 - 0.5)

### 8. Dropout2D (Spatial Dropout)

Версия Dropout для сверточных слоев, которая выключает целые каналы целиком.

**Параметры:**
- **Prob**: Вероятность исключения канала (от 0.0 до 1.0)

### 9. GaussianNoise (Гауссово шум)

Добавляет случайный шум к активациям во время обучения, действуя как форма регуляризации.

**Параметры:**
- **Std Dev**: Стандартное отклонение шума (обычно 0.01 - 0.1)

### 10. Add Node (Слой сложения)
