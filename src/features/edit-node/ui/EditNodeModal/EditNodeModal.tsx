...
import styles from './EditNodeModal.module.css';
...
export const EditNodeModal: React.FC<EditNodeModalProps> = ({
  ...
  const loadNodeParameters = (node: BaseNode) => {
    ...
    if (node instanceof InputNode) {
      ...
      setConvActivation(params.activation || 'relu');
  ...
  const handleSave = () => {
    try {
      ...
      if (existingNode && !existingNode.next.every((val: BaseNode) => newNode.CheckCompabilityDisconnected(val))) {
